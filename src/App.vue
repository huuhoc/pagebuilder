...
<template>
  <div id="aqt-contain" class="flex flex-1 bg-gray-100 overflow-hidden" ref="fullscreenElement">
      ...
      <div class="overflow-hidden h-full flex flex-col">
        ...
        <div class="flex-1 overflow-x-auto">
          <Container behaviour="copy" group-name="list-elements" :get-child-payload="layoutStore.getChildPayload1">
            <Draggable v-for="item in layoutStore.availableElements" :key="item.id" class="aqt-sidebar__element">
    ...
    <div class="w-10/12 flex flex-col relative">
      <div class="flex items-center justify-end sticky right-0 bg-gray-100 z-30 top-0 py-1 px-2.5">
        <span class="cursor-pointer p-2" :title="isFullScreen ? 'Đóng toàn màn hình' : 'Toàn màn hình'"
      ...
      <div class="flex-1 flex flex-col space-y-2 p-2.5 overflow-y-auto">
        <div class="flex-1 relative">
          <div v-if="layoutStore.layoutElements && layoutStore.layoutElements.length < 1"
            class="absolute inset-0 flex items-center justify-center">
            <p class="text-center">Không có dữ liệu</p>
          </div>
          <Container v-else :get-child-payload="layoutStore.getChildPayload2" :autoScrollEnabled="true"
            @drop="layoutStore.onDropLayout($event)" drag-handle-selector=".drag-handle" lock-axis="y"
            class="space-y-2">
            <Draggable v-for="item in layoutStore.layoutElements" :key="item.id" class="flex">
              <component :is="resolveComponent(item.el)" :dataItem="item"
  ...
  <ConfigModal v-if="settingsElementStore.showSettingsElement" :is-open="settingsElementStore.showSettingsElement" />
</template>
